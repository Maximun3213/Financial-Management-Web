...
const PageCycleContext = createContext<PageCycleContextType | undefined>(undefined);

export function PageCycleProvider({ children }: { children: ReactNode }) {
  const pageStatus = useSignal(PageStatus.IDLE);
  ...
  const isPageEntering = useComputed(() => pageStatus.value === PageStatus.ENTERING);
  const isPageEntered = useComputed(() => pageStatus.value === PageStatus.ENTERED);
  const isPageExiting = useComputed(() => pageStatus.value === PageStatus.EXITING);
  const isPageExited = useComputed(() => pageStatus.value === PageStatus.EXITED);
  ...
  return <PageCycleContext.Provider value={contextValue}>{children}</PageCycleContext.Provider>;
}
